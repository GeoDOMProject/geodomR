...
remotes::install_github("GeoDOMProject/geodomR@v1.1.0")
```
...
```r
library(geodom)
datos <- data.frame(PROV_CODE = c("01", "25"), valor = c(0, 34))
gd_map_interactive(datos, fill = "valor", .level = "provinces",
                   .name = "PROV_CODE", .key = "PROV_CODE", file = "mapa.html")
```

El resultado es un documento `geodom_html`. `file` guarda el documento sin abrir
el navegador automáticamente; también puede guardarse con `writeLines()`.
`context = FALSE` omite las capas adicionales.
